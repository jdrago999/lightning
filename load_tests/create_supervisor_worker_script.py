...
program_template = """
[program:lightning_worker-$port]
environment=NEW_RELIC_CONFIG_FILE='/var/www/lightning/conf/newrelic-preprod.ini'
command=newrelic-admin run-program python /var/www/lightning/bin/worker --queue=Service --log=error --server=fake-lightningpre1..local:6379 --config=/var/www/lightning/conf/lightning-preprod.conf --play=1 --use_filters=1 -e 1
